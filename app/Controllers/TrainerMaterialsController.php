...
class TrainerMaterialsController {
  ...
  public function modules(){
    $this->ensureTrainer();
    $pdo = DB::conn(); $course = $_GET['course'] ?? '';
    // Using existing Modules table; treat 'content' as description and require moduleTitle column if available
    try{
      $mods = $pdo->prepare("SELECT moduleID, COALESCE(moduleTitle, content) AS title, content AS description, 1 AS position FROM Modules WHERE courseID=? ORDER BY moduleID ASC");
      $mods->execute([$course]); $modules = $mods->fetchAll();
    }catch(Throwable $e){ $modules=[]; }
    return render('trainer/materials/modules', ['courseID'=>$course,'modules'=>$modules,'errors'=>$_SESSION['errors']??[]]);
  }
