<section class="mt-6">
  ...
  <div class="card" style="padding:16px; margin-top:14px;">
    <div class="flex-between" style="margin-bottom:8px;">
      <h3 style="margin:0;">Pipeline quick filters<?= !empty($latestJobTitle) ? ' — <span class=\"muted\">'.e($latestJobTitle).'</span>' : '' ?></h3>
      <?php if(empty($latestJobID)): ?><small class="muted">No jobs yet</small><?php endif; ?>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <a class="btn btn-outline btn-sm<?= empty($latestJobID)?' disabled':'' ?>" href="<?= !empty($latestJobID)? app_url('employer/jobs/applicants').'?job='.e($latestJobID).'&status=Under+Review' : '#' ?>">Under Review</a>
      <a class="btn btn-outline btn-sm<?= empty($latestJobID)?' disabled':'' ?>" href="<?= !empty($latestJobID)? app_url('employer/jobs/applicants').'?job='.e($latestJobID).'&status=Interview' : '#' ?>">Interview</a>
      <a class="btn btn-outline btn-sm<?= empty($latestJobID)?' disabled':'' ?>" href="<?= !empty($latestJobID)? app_url('employer/jobs/applicants').'?job='.e($latestJobID).'&status=Hired' : '#' ?>">Hired</a>
    </div>
  </div>

  <div class="card" style="padding:16px; margin-top:14px;">
    <h3 style="margin:0 0 8px;">Conversion snapshot</h3>
    <div class="grid" style="display:grid;grid-template-columns:repeat(3,minmax(140px,1fr));gap:10px;">
      <div class="card" style="padding:12px;">
        <div class="muted">Applicants (7d)</div>
        <div style="font-size:22px;font-weight:800;">&nbsp;<?= (int)$apps7 ?></div>
      </div>
      <div class="card" style="padding:12px;">
        <div class="muted">Interviews (7d)</div>
        <div style="font-size:22px;font-weight:800;">&nbsp;<?= (int)$iv7 ?></div>
      </div>
      <div class="card" style="padding:12px;">
        <div class="muted">Hires (7d)</div>
        <div style="font-size:22px;font-weight:800;">&nbsp;<?= (int)$hi7 ?></div>
      </div>
      <div class="card" style="padding:12px;">
        <div class="muted">Applicants (30d)</div>
        <div style="font-size:22px;font-weight:800;">&nbsp;<?= (int)$apps30 ?></div>
      </div>
      <div class="card" style="padding:12px;">
        <div class="muted">Interviews (30d)</div>
        <div style="font-size:22px;font-weight:800;">&nbsp;<?= (int)$iv30 ?></div>
      </div>
      <div class="card" style="padding:12px;">
        <div class="muted">Hires (30d)</div>
        <div style="font-size:22px;font-weight:800;">&nbsp;<?= (int)$hi30 ?></div>
      </div>
    </div>
  </div>
      ...
      <div style="flex:1;min-width:260px;">
        <form method="post" action="<?= app_url('employer/profile/update') ?>" enctype="multipart/form-data" style="display:grid;gap:8px;">
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap;">
            ...
            <a class="btn btn-outline" href="<?= app_url('employer/jobs/create') ?>">Post Job</a>
            <a class="btn" href="<?= app_url('employer/jobs') ?>">My Job Listings</a>
            <?php
              $shortCount = 0; try{ $sp = dirname(__DIR__,3).'/storage/shortlist_'.$eid.'.json'; if(file_exists($sp)){ $data = json_decode(file_get_contents($sp), true) ?: []; $shortCount = count(array_keys(array_filter($data))); } }catch(Throwable $e){}
            ?>
            <a class="btn btn-outline" href="<?= app_url('employer/talent') ?>">Talent Search<?= $shortCount>0 ? ' <span style="display:inline-block;margin-left:6px;padding:0 8px;border-radius:999px;background:#22c55e;color:white;font-size:12px;">'.$shortCount.'</span>' : '' ?></a>
          </div>
        </form>
      </div>
    </div>
